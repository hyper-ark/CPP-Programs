...
Image::Image(const Image& other) : image(nullptr), width(0), height(0) {
    this->allocateImage(other.width, other.height);
    for (unsigned int i = 0; i < width; i++) {
        for (unsigned int j = 0; j < height; j++) {
            image[i][j].setRed(other.image[i][j].getRed());
            image[i][j].setGreen(other.image[i][j].getGreen());
            image[i][j].setBlue(other.image[i][j].getBlue());
        }
    }
}

Image& Image::operator=(const Image& copy) {
    this->allocateImage(copy.width, copy.height);
    for (unsigned int i = 0; i < width; i++) {
        for (unsigned int j = 0; j < height; j++) {
            image[i][j].setRed(copy.image[i][j].getRed());
            image[i][j].setGreen(copy.image[i][j].getGreen());
            image[i][j].setBlue(copy.image[i][j].getBlue());
        }
    }
    return *this;    
}
